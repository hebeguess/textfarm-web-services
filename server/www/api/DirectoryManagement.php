					...
					if (mysql_errno($dbCon) == '0' and mysql_num_rows($_result) > 0) { // IF user has contents stored
						...
						while($_row = mysql_fetch_array($_result))
						{
							if ($_row[3] == '1') { // filePermission = 'public'
								...
								if ($row[1] != '') { // IF publicPassphrase existed, generate direct link
									$xml .= ">\n\t\t\t<link url=\"http://$_SERVER[HTTP_HOST]/api/content/retrieve/".$_row[0]."/".$row[1]."/\" />";
									$xml .= "\n\t\t</content>";
								}
								else // IF publicPassphrase not exist
								{
							...
							else // filePermission = 'private'
							{
								...
								if (mysql_errno($dbCon) == '0' and mysql_num_rows($__result) > 0) { // IF content have any authKeys assignment
									...
									while($__row = mysql_fetch_array($__result)) // loop contents
									{
										if ($__row[1] == 0) {
											$xml .= "\n\t\t\t<authorize authKeys=\"".$__row[0]."\" status=\"active\"";

											if ($row[1] != '') { // IF publicPassphrase & authKeys is 'active' existed, generate direct link
												$xml .= ">\n\t\t\t\t<link url=\"http://$_SERVER[HTTP_HOST]/api/content/retrieve/".$__row[0]."/".$row[1]."/\" />";
												$xml .= "\n\t\t\t</authorize>";
											}
											else
											{ // IF publicPassphrase not exist
												$xml .= " />";
											}		
										}
										else
										{
											$xml .= "\n\t\t\t<authorize authKeys=\"".$__row[0]."\" status=\"inactive\" />";
										}
									}
									$xml .= "\n\t\t</content>";																		
								}								
